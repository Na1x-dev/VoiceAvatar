...
// Устанавливаем новый диапазон до 300%
sizeSlider.max = 300;
...
// Функции для шаговых кнопок громкости (+ и -)
window.stepBaseThreshold = async function (key, direction) {
    const slider = document.getElementById(`slider-base-${key}`);
    if (!slider) return;
    let newVal = parseInt(slider.value) + direction;
    if (newVal >= 1 && newVal <= 255) {
        slider.value = newVal;
        await updateBaseThreshold(key, newVal);
    }
};

window.stepCustomProp = async function (index, direction) {
    const slider = document.getElementById(`slider-custom-${index}`);
    if (!slider) return;
    let newVal = parseInt(slider.value) + direction;
    if (newVal >= 1 && newVal <= 255) {
        slider.value = newVal;
        await updateCustomProp(index, newVal);
    }
};
...
window.updateBaseThreshold = async function (key, value) {
    ...
    const indicator = document.getElementById(`vol-base-val-${key}`);
...
window.updateCustomProp = async function (index, value) {
    ...
    const indicator = document.getElementById(`vol-val-${index}`);
    if (indicator) indicator.innerText = value;
    await saveToDB();
};
